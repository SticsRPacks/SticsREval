...
make_eval_stats <- function() {
  data.frame(
    situation = c("usm1", "usm1", "usm2"),
    variable  = c("LAI", "MASEC", "LAI"),
    rRMSE     = c(0.12, 0.18, 0.30)  # LAI usm1: +20%, MASEC: -10%, LAI usm2: +100%
  )
}

make_comparison <- function(percentage = 10, species = "wheat",
...
test_that("errors when neither data nor species/ref/eval provided", {
  expect_error(
...
test_that("data is sorted descending by ratio", {
  ...
  expect_equal(ratios, sort(ratios, decreasing = TRUE))
})

# ---- ratio computation ----

test_that("computes ratio correctly", {
  ref  <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.10)
  eval <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.15)
  ...
  expect_equal(cmp$get_data()$ratio, 50)
})
...
test_that("filters out non-finite and NA rows", {
  ...
  expect_equal(nrow(cmp$get_data()), 1)
})
...
test_that("warning_vars returns variables between 0 and percentage", {
  ref  <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.10)
  eval <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.105)
  cmp <- RmseComparison$new(
    percentage = 10, species = "wheat",
    ref_stats = ref, eval_stats = eval
  )
...
test_that("is_empty returns TRUE when no rows", {
  ...
  ref  <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.10)
  eval <- data.frame(situation = "usm2", variable = "LAI", rRMSE = 0.12)
  cmp <- RmseComparison$new(
    percentage = 10, species = "wheat",
    ref_stats = ref, eval_stats = eval
  )
...
test_that("get_data returns expected columns", {
  cmp <- make_comparison()
  expected_cols <- c("species", "situation", "variable", "rmse_new", "rmse_ref", "ratio")
  expect_named(cmp$get_data(), expected_cols)
})

# ---- DeterioratedUSMComparison ----

test_that("DeterioratedUSMComparison keeps only deteriorated rows (ratio > 0)", {
...
test_that("DeterioratedUSMComparison is empty when all ratios are improved", {
  ref  <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.20)
  eval <- data.frame(situation = "usm1", variable = "LAI", rRMSE = 0.10)
